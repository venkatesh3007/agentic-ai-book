...
function recordIssue(file, type, message) {
  issues.push({ file, type, message });
  console.error(`ERROR [${type}] ${file} — ${message}`);
}

function recordWarning(file, type, message) {
  warnings.push({ file, type, message });
...
function writeReports(summary) {
  ...
  md.push('- Warns when chapter files are missing a `subtitle`.');
